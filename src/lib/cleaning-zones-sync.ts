...
export type CleaningZonesSyncResult = {
  upserted: number;
  skipped: number;
  errors: string[];
};
...
/**
 * Fetch CleaningZones from data.goteborg.se and upsert into `cleaning_zones` via
 * `upsert_cleaning_zone_from_sync` (service role).
 *
 * Env: GOTHENBURG_DATA_API_KEY, NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
 */
export async function syncCleaningZonesFromGothenburg(): Promise<CleaningZonesSyncResult> {
